...
if ($obj['email'] != "") {
	...
	if ($result->num_rows > 0) {
		...
		//    $add = $mysqli->query("insert into users (name,email,password,userType) values('$name','$email','$userType','$password')");
		$query = sprintf(
			"insert into users (name,email,userType,password) values('%s','%s','%s','%s')",
